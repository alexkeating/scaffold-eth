...
describe("SuperswapV1Factory", () => {
  ...
  async function createPair(tokens: [string, string]) {
    const bytecode = `${SuperswapV1Pair.bytecode}`;
    ...
    await expect(factory.createPair(...tokens)).to.be.reverted; // SuperswapV1: PAIR_EXISTS
    await expect(factory.createPair(...tokens.slice().reverse())).to.be
      .reverted; // SuperswapV1: PAIR_EXISTS
  ...
  it("createPair:gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq(2022206); // Changed gas
  });

  it("setFeeTo", async () => {
    await expect(
      factory.connect(other).setFeeTo(other.address)
    ).to.be.revertedWith("SuperswapV1: FORBIDDEN");
